...
async function getWishlistMemberByUserIdAndWishlistId(userId, wishlistId) {
    const member = WishlistModel.findWishlistMemberByUserIdAndWishlistId(userId, wishlistId)
    console.log(member)
    return member
}
 ...
 */
async function verifyWishlistRoleByWishlistId(userId, wishlistId, requiredWishlistRole) {
    const member = await getWishlistMemberByUserIdAndWishlistId(userId, wishlistId)
    if (!member.hasRole(requiredWishlistRole)) {
        throw new ForbiddenError(`User with the id ${userId} hasnt ${requiredWishlistRole.roleName} for the wishlist with id ${wishlistId}`)
    }
}
 ...
 * @throws {ForbiddenError}
 */
async function verifyWishlistRoleByWishlistItemId(userId, wishlistItemId, requiredWishlistRole) {
    const wishlistId = await getWishlistItemById(wishlistItemId)
    await verifyWishlistRoleByWishlistId(userId, wishlistId, requiredWishlistRole)
}
