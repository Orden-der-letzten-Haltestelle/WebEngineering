...
export async function buyCart(token) {
    const res = await fetch(`${config.host}/cart/buy/`, {
        method: "POST",
        headers: {
            Authorization: token,
        },
    ...
    if (!res.ok) {
        ...
        const errorMessage = errorData.message || "Failed to buy cart"
...
export async function updateAmount(cartItemId, newAmount, token) {
    const res = await fetch(
        `${config.host}/cart/item/${cartItemId}?amount=${newAmount}`,
        {
            method: "PUT",
            headers: {
                Authorization: token,
